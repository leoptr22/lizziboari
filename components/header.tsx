          ...
          <div className="hidden md:flex items-center gap-10">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-[11px] uppercase tracking-[0.2em] text-zinc-500 hover:text-zinc-900 transition-colors relative group"
              >
                {link.label}
            ...
            <div className="flex flex-col py-6 shadow-xl">
              {navLinks.map((link) => (
                <Link
                  ...
                  className="px-8 py-4 text-xs tracking-widest text-zinc-600 hover:text-zinc-900 hover:bg-zinc-50 transition-all"
                >
                  {link.label}
                </Link>
              ))}
